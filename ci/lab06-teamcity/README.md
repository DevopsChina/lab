...
不论项目再怎么复杂，为项目建立发布流水线的第一步，就是先用 **人工** 把所有构建任务运行一次，并将所有要运行的命令手写下来。以一个用 Kotlin 编程语言撰写的 ShoppingCart 项目为例，流水线的设计如下：

* 以 Gradle 运行构建并生成 Artifact
* 以 kotest 运行测试并生成覆率报告
* 以 ktlint 检查 Coding Style
* 以 Qodana 或 detekt 运行静态分析
* 以 Dokka 生成 API 文件
* 发布 Package
* 部署 API 文件
...
有了上面的蓝图后，接下来会花三个 Lab 逐步以 TeamCity 将发布流水线搭建起来。第一步要先在 TeamCity 里完成第一个构建，确认项目可顺利编译。
...
* TeamCity 支持从 GitHub、Bitbucket Cloud、GitLab 引入代码仓库，或是提供 Repository URL 也行。练习时可直接使用笔者的[演示 Repo](https://github.com/shengyou/shopping-cart)，输入后按 Process。
...
* 有了 Build Configuration 后，接着要设置 Build Step，也就是构建过程中要运行的任务步骤。TeamCity 会自动扫描 Repository 使用的开发工具，扫描后列出可运行的 Build Step。以演示 Repo 为例，TeamCity 发现有两个动作可以做，一个是 Gradle、一个是 Command Line。但我们只需要 Gradle 即可，勾选 Gradle 选项，点 Use selected 进到下一步。
  ![](images/lab1/lab1-step4.png)
* 在构建完成后，要将生成的 Artifact 储存下来。回到 Build 的 General Settings 设置，在 Artifact paths 设置要储存的文件夹路径。
  ![](images/lab1/lab1-stepx.png)
* 完成后就可以点选划面右上方的 Run 按钮执行第一次的建置工作。
  ![](images/lab1/lab1-step6.png)
* TeamCity 会自动跳转至构建运行的页面，我们可以即时看到 TeamCity 正在迟行的任务，TeamCity 会以时间线显示构建过程，并可实时查看终端输出。若构建过程没有发生错误，则那这个建置就会被视为成功，您可以看到 TeamCity 会以绿色字及打勾的图标来表示构建结果。
...
第二个 Lab 要依照流水线计划增加更多 Build Step，包括以 ktlint 检查 Coding Style，以 Qodana 或 detekt 运行静态分析，并为测试生成覆盖率报告。

* 转至项目设置，进入 Build Step 设置，新增一个 Build Step。
  ![](images/lab2/lab2-step1.png)
* 由于演示 Repo 使用 Gradle 集成 ktlint，Runner Type 可使用 Gradle Runner。Step name 可命名为 Check Code Style，Gradle tasks 里填入要运行的命令 `lintKotlin`，其余留空按 Save 储存即可。
...
第三个 Lab 要依照流水线计算生成项目的 API 文件，文件部署至指定服务器。演示 Repo 使用的是 Dokka 文件引擎，由于 Dokka 也有提供 Gradle 插件，因此在 TeamCity 里一样使用 Gradle Runner 即可生成文件。

* 转至项目 Build Step 设置，新增一个 Step，Runner Type 选 Gradle、Step name 输入 Generate API Document、Gradle tasks 输入 `dokkaHtml`，其他保留默认后按 Save 储存。
...
与前面构建的步骤不同，部署文件的任务不需要跟构建绑在一起，可以单独创建一个 Build Configuration，等需要部署时再「人工部署」。
...
* 上一步只把 API 文件的压缩档上传，还没办法让用户浏览。所以要搭配第二步动作，直接 SSH 进服务器，把压缩档解开放到 Nginx Site 底下。回到 Build Steps 设定再新增第二个 Build Step，Runner type 选 SSH Exec，Step name 取名为 Publish，Target、Port、Username 请依照目标服务器设置，Commands 就直接把在服务器上运行的 Shell Script 直接黏贴上去，设定好后按 Save 储存。
